...
const Select: React.FC<SelectProps> = ({ label = "", name, value, options, className = "", onChange }) => {
    ...
    const labelClassName = "block font-medium text-secondary-700 dark:text-white mb-1";
    const selectTriggerClassName = "text-xs md:text-base transition-all duration-200 w-full px-3 py-2 dark:text-white bg-white/50 dark:bg-black/50 border dark:border-secondary-700 border-secondary-300 rounded-lg shadow-sm backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 flex justify-between items-center cursor-pointer space-x-2";
