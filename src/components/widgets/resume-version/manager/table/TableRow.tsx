...
import TableColumnActions from "./row/TableColumnActions";
import TableColumnName from "./row/TableColumnName";
...
const TableRow: React.FC<TableRowProps> = ({ currentResumeId, version }) => {
    ...
    const classNameNormal = `flex items-center w-full px-3 py-2 transition-all text-sm md:text-base lg:text-lg whitespace-nowrap border-b-4`;
    ...
    return (
        <tr key={version.id} className={`${classNameStatus(version)} ${classNameNormal}`}>
